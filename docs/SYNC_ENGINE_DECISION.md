...
- Engine ownership in `planner_config.json` (`"engine": "undated"`) is already claimed/asserted by the hardened engine (Phase 0 above); what remains here is the legacy side: `SyncService`, if ever re-enabled for the Inbox lane, must refuse to push while the marker names another engine. This protects against two *installations* at different app versions writing the same list.
