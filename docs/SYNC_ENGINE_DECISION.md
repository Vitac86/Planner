...
*Status: runbook and operator tooling implemented (July 2026, `tests/test_undated_pilot.py`); the pilot itself has not been executed yet. Nothing in this phase runs automatically, the default engine remains `"legacy"`, and no real-account data is migrated.*

- **Runbook: `docs/UNDATED_SYNC_PILOT_RUNBOOK.md`** — the step-by-step operator guide: isolating a pilot profile (separate data dir via the `APPDATA`/`XDG_DATA_HOME` override, own `token.json`, never the real one), seeding legacy tasks, the backup → export → dry-run → review → apply → flag-flip sequence, the functional checklist, two-device simulation, log/appData inspection, rollback, and the explicit real-account don'ts.
- **Operator script: `scripts/undated_migration_pilot.py`** — `backup` / `export` / `dry-run` / `apply` / `verify` (`python -m scripts.undated_migration_pilot <command>`). Dry-run/read-only by default; `apply` demands existing backup **and** export evidence paths (the `confirm_without_backup` waiver is not exposed); it never starts the Flet UI/`AppShell`, never sets `PLANNER_UNDATED_ENGINE`, never deletes remote Google Tasks, and never clears `Task.gtasks_id`. `verify` cross-checks local tasks vs. `SyncMapUndated` vs. the shared index (missing mappings, uid mismatches, missing/duplicate index entries, preserved `gtasks_id`, engine marker, evidence paths).
- **Exact pilot checklist** (runbook §9, each item observed, not assumed): create/edit/complete/delete in both directions; priority change on device A visible on device B (two data dirs with distinct `device_id.txt` simulate two devices); schedule an undated task → Google Task disappears, Calendar event appears; unschedule → reverse; offline edits queue and drain; kill the app mid-sync and verify convergence on restart; confirm `gtasks_index.json` contents via the export/inspection command; rehearse rollback to `"legacy"` and re-verify.
- **Hard requirement:** the **first pilot uses a test Google account only** — never the developer's real account. The real-account cutover (and only then flipping the default flag to `"undated"`) is a separate, later decision taken after the pilot checklist passes end-to-end.
