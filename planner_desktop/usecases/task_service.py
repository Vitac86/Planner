...
INTERACTION_SAVE_ERROR = "Не удалось сохранить изменение календаря"
TIMED_TASK_REQUIRED_ERROR = "Для этого действия нужна задача со временем."
UNDATED_TASK_REQUIRED_ERROR = "Эта задача уже запланирована."
DUPLICATE_DELETED_ERROR = (
    "Удалённую задачу нельзя дублировать — сначала восстановите её."
)
SERIES_INTERACTION_ERROR = (
    "Измените экземпляр серии через редактор и выберите область изменений"
)
SERIES_SCOPE_REQUIRED_ERROR = (
    "Это экземпляр локальной серии: выберите область изменений "
    "(«только этот» или «этот и все будущие») в редакторе."
)
...
class DesktopTaskService:
    ...
    def __init__(
        ...
        # Optional local-only services. Kept loosely typed to avoid a
        # domain/usecase import cycle; MainWindow attaches them after the
        # adapters are built. None в тестах/демо — поверхности деградируют
        # мягко (нет серий/шаблонов, но CRUD работает).
        self.tag_service = tag_service
        self.recurrence_service = None
    ...
    @staticmethod
    def _is_local_series_occurrence(task: Task) -> bool:
        """Экземпляр локальной серии: Calendar-очередь для него закрыта.

        Без этой проверки record_local_update поставил бы create для любой
        датированной задачи без event_id — и каждый материализованный
        экземпляр улетел бы в Google отдельным событием (Phase 3.2A
    ...
    def update_task(self, task: Task) -> Task:
        ...
        if self._queue is not None and not self._is_local_series_occurrence(updated):
    ...
    def delete_task(self, task_id: int) -> bool:
        ...
        try:
            ...
            if (
                ...
                and not self._is_local_series_occurrence(task)
            ):
    ...
    def duplicate_task(self, uid: str) -> TaskOperationResult:
        """Create an independent ordinary copy through normal create semantics."""

        source = self.repository.get_by_uid(uid)
        if source is None:
            return TaskOperationResult(errors=[TASK_NOT_FOUND_ERROR])
        if source.is_deleted:
            return TaskOperationResult(errors=[DUPLICATE_DELETED_ERROR])
        duplicate = Task(
            title=source.title,
            notes=source.notes,
            tags=tuple(source.tags),
            start=deepcopy(source.start),
            end=deepcopy(source.end),
            duration_minutes=source.duration_minutes,
            is_all_day=source.is_all_day,
            priority=source.priority,
            completed=False,
            completed_at=None,
            # uid, all Google ids/etag/recurrence fields, timestamps and
            # tombstone intentionally come from fresh Task defaults.
        )
        try:
            created = self.create_task(duplicate)
    ...
    def edit_task(self, uid: str, command: TaskEditorCommand) -> TaskOperationResult:
        ...
        if self._is_local_series_occurrence(task):
            # Экземпляры локальных серий правятся только через
            # RecurrenceService с явной областью изменений.
            return TaskOperationResult(errors=[SERIES_SCOPE_REQUIRED_ERROR])
    ...
    # ---- Phase 2.2 Calendar interactions ---------------------------------------
    ...
    @staticmethod
    def _schedule_tuple(task: Task) -> tuple:
        return (task.start, task.end, task.duration_minutes, task.is_all_day)

    @staticmethod
    def _copy_task_state(source: Task, target: Task) -> None:
        for descriptor in fields(Task):
            setattr(target, descriptor.name, deepcopy(getattr(source, descriptor.name)))

    def _queue_snapshot(self, uid: str):
        if self._queue is not None and hasattr(self._queue, "snapshot_task_ops"):
            return self._queue.snapshot_task_ops(uid)
        return None

    def _restore_queue_snapshot(self, uid: str, snapshot) -> None:
        if (
            snapshot is not None
            and self._queue is not None
            and hasattr(self._queue, "restore_task_ops")
        ):
            self._queue.restore_task_ops(uid, snapshot)

    def _restore_repository_task(self, snapshot: Task) -> None:
        current = self.repository.get_by_uid(snapshot.uid)
        if current is None:
            return
        self._copy_task_state(snapshot, current)
        self.repository.update(current)

    def _commit_schedule_change(
        self,
        task: Task,
        *,
        ...
        """Persist one interaction and compensate both stores on failure."""
        original = deepcopy(task)
        proposed = (start, end, duration_minutes, is_all_day)
        if proposed == self._schedule_tuple(task):
            return TaskOperationResult(task=task)
        queue_snapshot = self._queue_snapshot(task.uid)
        try:
            task.start = start
            ...
            if self._queue is not None:
                if unschedule:
                    if original.google_calendar_event_id is not None:
                        self._queue.enqueue_delete(
                            task.uid,
                            payload={"event_id": original.google_calendar_event_id},
                        )
                    else:
                    ...
                    record_local_update(self._queue, updated)
            return TaskOperationResult(task=updated)
        except Exception as exc:  # repository/queue adapters define failures
            ...
            try:
                ...
                rollback_errors.append(str(rollback_exc))
            message = f"{INTERACTION_SAVE_ERROR}: {exc}"
            if rollback_errors:
                message += " (ошибка отката: " + "; ".join(rollback_errors) + ")"
            return TaskOperationResult(errors=[message])

    def _validate_timed_interval(
        self, start: datetime, end: datetime
    ) -> Optional[str]:
        minutes = int((end - start).total_seconds() // 60)
        if minutes < MIN_TIMED_DURATION_MINUTES:
            return (
                f"Минимальная длительность — {MIN_TIMED_DURATION_MINUTES} минут"
            )
        if minutes > MAX_DURATION_MINUTES:
            return f"Длительность не может превышать {MAX_DURATION_MINUTES} минут"
        return None
    ...
    def move_timed_task(
        ...
        *,
        ...
        current_minutes = (
            ...
            if task.end is not None and task.end > task.start
            else task.duration_minutes or DEFAULT_TIMED_DURATION_MINUTES
        )
        minutes = duration_minutes if duration_minutes is not None else current_minutes
        proposed_end = end or start + timedelta(minutes=minutes)
        error = self._validate_timed_interval(start, proposed_end)
        if error:
            return TaskOperationResult(errors=[error])
        minutes = int((proposed_end - start).total_seconds() // 60)
    ...
    def resize_timed_task(
        ...
        *,
        ...
        proposed_end = end or task.end
        if proposed_end is None:
            proposed_end = proposed_start + timedelta(
                minutes=task.duration_minutes or DEFAULT_TIMED_DURATION_MINUTES
            )
        if proposed_start.date() != proposed_end.date():
            return TaskOperationResult(errors=[
                "Изменение многодневных событий пока не поддерживается"
            ])
        error = self._validate_timed_interval(proposed_start, proposed_end)
        if error:
            return TaskOperationResult(errors=[error])
        minutes = int((proposed_end - proposed_start).total_seconds() // 60)
        return self._commit_schedule_change(
            task,
    ...
    def convert_to_timed(
        self,
        uid: str,
        start: datetime,
        *,
        duration_minutes: int = DEFAULT_TIMED_DURATION_MINUTES,
    ) -> TaskOperationResult:
        task = self.get_task(uid)
        if task is None:
            return TaskOperationResult(errors=[TASK_NOT_FOUND_ERROR])
    ...
    def schedule_undated_task(
        ...
        *,
        ...
        if task.start is not None:
            return TaskOperationResult(errors=[UNDATED_TASK_REQUIRED_ERROR])
        if self._is_recurring_instance(task):
            return TaskOperationResult(errors=[RECURRING_INTERACTION_ERROR])
        if self._is_local_series_occurrence(task):
            return TaskOperationResult(errors=[self._series_interaction_error(task)])
        if is_all_day:
            ...
            return self._commit_schedule_change(
                task,
                start=all_day_start,
                end=end or all_day_start + timedelta(days=1),
                duration_minutes=None,
                is_all_day=True,
            )
        return self.convert_to_timed(
            uid, start, duration_minutes=duration_minutes
        )

    def apply_drag_proposal(
        self, proposal: CalendarDragProposal
    ) -> TaskOperationResult:
        if not proposal.valid:
            return TaskOperationResult(errors=[proposal.message])
        task = self.get_task(proposal.task_uid)
        if task is None:
            return TaskOperationResult(errors=[TASK_NOT_FOUND_ERROR])
    ...
    def apply_resize_proposal(
        self, proposal: CalendarResizeProposal
    ) -> TaskOperationResult:
        if not proposal.valid:
            return TaskOperationResult(errors=[proposal.message])
        task = self.get_task(proposal.task_uid)
