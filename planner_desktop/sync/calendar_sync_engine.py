...
- «расписание снято» (задача имела событие, но start обнулили) пока не
  реализовано — в UI нет действия unschedule; операция тихо снимается
  с очереди, событие остаётся в календаре (задокументированный пробел);
...
class CalendarSyncEngine:
    ...
    def _push_op(self, op: PendingOp) -> None:
        ...
        if task.is_deleted:
            ...
            return
        if task.start is None:
            # Unschedule не реализован (см. докстринг модуля) — операция
            # снимается, событие в календаре не трогается.
