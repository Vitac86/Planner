...
| Изолированный UI smoke + persistence после restart | PASS | 3 режима создания, 4 перехода расписания, 6 пресетов, 5 snooze-путей, complete/restore, delete confirmation, обе цели shortcuts и 3 layout-режима; после перезапуска обе оставленные задачи перечитаны из `D:\Users\v.pyatakov\myspace\planner-desktop-ui-smoke` |
...
- профиль `D:\Users\v.pyatakov\myspace\planner-desktop-calendar-smoke`: 14 synthetic tasks,
...
Fake/injected smoke в `D:\Users\v.pyatakov\myspace\planner-desktop-google-series-write-smoke`
...
`D:\Users\v.pyatakov\myspace\planner-desktop-google-series-live-pilot` (собственные client_secret,
...
`D:\Users\v.pyatakov\myspace\planner-desktop-google-series-live-pilot` (собственные `client_secret.json`,
...
### Фаза 3.2B3B — individual occurrence sync (реализовано; live-пилот PASS)

- schema v10: отдельная occurrence-link таблица, очередь instance UPDATE/CANCEL
  и durable resolution-поля quarantine; миграция additive/idempotent;
- строгая identity `(series_uid, occurrence_key)` ↔
  `(recurringEventId, originalStartTime)`, без поиска по перемещённому start;
- full-resource instance update/cancel с сохранением чужих полей Google,
  Planner private markers, current ETag и повторной проверкой перед записью;
- поддержаны title/notes, same-kind timed move/resize, all-day date move и
  tombstone cancellation только одного экземпляра; master RRULE не меняется;
- очередь coalesce-ит UPDATE+UPDATE и UPDATE+CANCEL, отделена от master и
  ordinary Task queues, terminal требует явного retry;
- pull распознаёт echo, changed/cancelled instance сохраняет в quarantine и
  никогда не импортирует как ordinary Task;
- явные Use Google / Keep Planner / Keep both locally / Ignore, включая
  second-edit ETag race protection и reconciliation после local persist failure;
- ручной порядок: master → occurrence → ordinary → pull; startup/page-open/
  timer sync остаётся выключен.

Архитектура: [GOOGLE_OCCURRENCE_SYNC_ARCHITECTURE.md](GOOGLE_OCCURRENCE_SYNC_ARCHITECTURE.md).

Контролируемый real-Google пилот **пройден 17 июля 2026 года** на личном
аккаунте, в `primary`, только из изолированного соседнего профиля
`D:\Users\v.pyatakov\myspace\planner-desktop-google-series-live-pilot` и после
явного подтверждения пользователя. Один будущий `COUNT=3` TEST-master
(Europe/Moscow, 15 минут, без гостей/чувствительных заметок) прошёл local
move+title, remote conflict, second-edit ETag race, повторный Keep Planner,
Use Google с нулевой записью, local cancel и принятие remote cancellation.
До cleanup master canonical content оставался неизменным; master PATCH и все
ordinary event writes равны нулю. После удаления TEST-master: active masters =
0, active instances = 0, ordinary TEST events = 0, все три очереди и terminal
= 0, unresolved quarantine = 0, `occurrence_event_flood=0`; restart-проверка
повторила нулевые локальные счётчики.

Первый запуск дошёл до завершения функционального сценария и удалённой
очистки, но сам harness ошибочно вызвал отсутствующий диагностический метод
master-store и завершился `AttributeError`. Отдельная проверка подтвердила
нулевые remote/local остатки; после исправления только cleanup-вызова новый
чистый preflight и полный повтор прошли. Обезличенный отчёт не содержит account
identity, OAuth secrets, token contents, remote IDs или payloads.

Fake-приёмка в `D:\Users\v.pyatakov\myspace\planner-desktop-occurrence-sync-smoke` — **PASS**:
ровно один instance UPDATE и один instance CANCEL, master ETag неизменен,
`ordinary_occurrence_event_flood=0`, ETag race superseded без overwrite,
Use Google без write, Keep both только локально, remote cancellation принят
как tombstone, restart persistence подтверждён, `qml_warnings=0`, opening UI
Google calls = 0. Ordinary Task sync и recurring-master sync сохранились.

Скриншоты fake-приёмки:

- [редактор экземпляра](screenshots/occurrence_edit_phase3_2b3b.png);
- [подтверждение переноса](screenshots/occurrence_move_phase3_2b3b.png);
- [отмена экземпляра](screenshots/occurrence_cancel_phase3_2b3b.png);
- [сравнение конфликта](screenshots/occurrence_conflict_phase3_2b3b.png);
- [карантин](screenshots/occurrence_quarantine_phase3_2b3b.png);
