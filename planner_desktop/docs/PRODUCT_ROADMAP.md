...
### Фаза 3.2B3B — individual occurrence sync (реализовано; live-пилот ожидает подтверждения)
...
Fake-smoke и финальные regression-счётчики фиксируются после завершения
приёмки. Реальный пилот не запускается без нового явного подтверждения.
