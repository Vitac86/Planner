...
## 14a. Phase 3.2B3B и граница Phase 3.2B3C

Phase 3.2B3B реализует отдельное linked-occurrence исключение: immutable
`occurrence_key` переводится только в точный Google `originalStartTime`, а
перемещённый current start никогда не является identity. Title/notes,
same-kind move/resize и cancellation создают отдельный instance UPDATE/CANCEL,
не master и не ordinary Task operation. Cancelled occurrence сохраняется как
локальный tombstone и durable remote instance identity.

Remote changed/cancelled instance не перезаписывает Task, а ждёт явного Use
Google / Keep Planner / Keep both locally / Ignore. Полное описание:
[`GOOGLE_OCCURRENCE_SYNC_ARCHITECTURE.md`](GOOGLE_OCCURRENCE_SYNC_ARCHITECTURE.md).

Phase 3.2B3C1 добавляет remote split «этот и будущие» для чистой
Planner-owned связанной серии: исходный мастер Google сокращается до
`COUNT = точное число слотов ДО целевого` (подсчёт только генератором,
раздел 4), преемник — второй мастер с DTSTART в целевом слоте и остатком
окончания (COUNT-остаток / исходный lossless UNTIL / never). Любое будущее
(>= цели) исключение/tombstone/quarantine/EXDATE блокирует split с точными
датами; прошлые исключения и история остаются у исходной серии. Полное
описание durable state machine, локальной атомарной финализации и rollback:
[`GOOGLE_SERIES_REMOTE_SPLIT_ARCHITECTURE.md`](GOOGLE_SERIES_REMOTE_SPLIT_ARCHITECTURE.md).

Phase 3.2B3C2 сохраняет adoption внешнего master, миграцию будущих
исключений на преемник, automatic merge, conversion между local/external
series и background sync.
