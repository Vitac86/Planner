...
2. Запустите десктоп на отдельном тестовом профиле:
   ...
   ```powershell
   $env:PLANNER_DESKTOP_DATA_DIR = "D:\Users\v.pyatakov\myspace\planner-desktop-pilot"
...
Phase 3.2B3C1: «Этот и будущие» для чистой Planner-owned связанной серии
создаёт durable-план разделения. Следующая ручная синхронизация сокращает
исходный мастер Google до прошлых экземпляров (`COUNT`) и создаёт ровно один
мастер-преемник с детерминированным id; после завершения в Google два
повторяющихся события. Будущие исключения блокируют split с точными датами
(ничего не удаляется молча); частично выполненный план можно явно откатить
из Settings. Adoption существующего внешнего master остаётся Phase 3.2B3C2.
