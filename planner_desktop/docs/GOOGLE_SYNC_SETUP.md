...
локальную серию; changed linked instance карантинится до Phase 3.2B3B.
...
«этот и все будущие» и разрешение карантина не реализованы (Phase
3.2B3B/3.2B3C). Открытие Settings/editor/диалогов по-прежнему читает только
SQLite; автоматического Google sync нет. Live-пилот Phase 3.2B3A на реальном
Calendar API — отдельная явно-подтверждаемая задача; текущая приёмка
выполнена только на FakeCalendarGateway.
